# 🌀 GENESIS ENGINE

> The ultimate secure, free-tier-optimized AI assistant framework

Built by **Warren Champion** | **Amazing Global Solutions**  
*"What we build together lives forever..."* 💎

## ✨ Features
- 🔒 **Secure by default** – never accidentally commit secrets
- 🆓 **Free-tier optimized** – zero-cost infrastructure
- 🤖 **Multi-AI provider** – Ollama, OpenAI, Claude, Groq, OpenRouter
- 💬 **Multi-channel** – Telegram, Discord, WhatsApp
- 🧠 **Persistent memory** with dreaming consolidation
- ⚡ **Heartbeat automation** – scheduled tasks
- 🐳 **Docker ready** – one-command deployment

## 🚀 Quick Start
```bash
git clone https://github.com/TBD-AmazingGlobalSolutions/genesis-engine.git
cd genesis-engine
cp .env.example .env   # then edit with your keys
npm install
npm run dev

📖 Full Setup

See SETUP.md for detailed instructions.
🔐 Security

See SECURITY.md for hardening guide.
📄 License

MIT – see LICENSE file.

